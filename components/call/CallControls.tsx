...
function ControlButton({ label, active, danger, badge, children, onClick, secondary = false, mobileOverflow = false }: { label: string; active?: boolean; danger?: boolean; badge?: number; children: React.ReactNode; onClick: () => void; secondary?: boolean; mobileOverflow?: boolean }) {
  return (
    <button type="button" className={`call-control ${active ? "is-active" : ""} ${danger ? "is-danger" : ""} ${secondary ? "is-secondary" : ""} ${mobileOverflow ? "is-mobile-overflow" : ""}`} onClick={onClick} aria-label={label} title={label}>
    ...
    <div className="call-controls-wrap">
      {moreOpen ? (
        <div className="more-menu" id="call-more-menu" role="menu">
          <button className="more-menu__mobile-only" type="button" role="menuitem" onClick={() => { props.onToggleReactions(); setMoreOpen(false); }}><Hand size={17} /> REACT</button>
      ...
      <nav className="call-controls" aria-label="Call controls">
        ...
        <ControlButton label="REACT" active={props.reactionOpen} onClick={props.onToggleReactions} mobileOverflow><Hand size={21} /></ControlButton>
